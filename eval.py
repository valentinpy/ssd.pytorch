...
def do_python_eval_kaist(output_dir='output', use_07=True):
    ...
    for i, cls in enumerate(labelmap):
        ...
        rec, prec, ap = kaist_eval(filepath, annopath, imgsetpath.format(set_type), cls, cachedir, ovthresh=0.5, use_07_metric=use_07_metric)
        aps += [ap]
        print('AP for {} = {:.4f}'.format(cls, ap))
        with open(os.path.join(output_dir, cls + '_pr.pkl'), 'wb') as f:
            pickle.dump({'rec': rec, 'prec': prec, 'ap': ap}, f)
...
if __name__ == '__main__':
    ...
    if args.dataset_type == "VOC":
        ...
        annopath = os.path.join(args.dataset_root, 'rgbt-ped-detection/data/kaist-rgbt/', '%s', 'annotations', '%s', '%s', '%s.txt')
        imgpath = os.path.join(args.dataset_root, 'rgbt-ped-detection/data/kaist-rgbt/', '%s', 'images', '%s', '%s', 'visible', '%s.jpg')
        imgsetpath = os.path.join(args.dataset_root, 'rgbt-ped-detection/data/kaist-rgbt/', '%s', 'imageSets', '%s.jpg')
